...
import CustomKline from './xueren/CustomKline.vue';
import PriceWatchChart from './price_watch/PriceWatchChart.vue';
...
SBA.use({
  install({ viewRegistry, i18n }) {
    ...
    viewRegistry.addView({
      name: "xueren-monitor",
      path: "/xueren",
      // 这里的 component 直接指向引入的 CustomKline
      component: CustomKline,
      label: "Xueren Stock",
      group: "xueren",
      order: 1000,
    });
    ...
    // 3. 注入多语言
    i18n.mergeLocaleMessage("en", { xueren: { label: "Xueren Stock" } });
    i18n.mergeLocaleMessage("zh", { xueren: { label: "雪人行情" } });
  }
});
...
SBA.use({
  install({ viewRegistry, i18n }) {
    ...
    // 3. 注入多语言
    i18n.mergeLocaleMessage("en", { price_watch: { label: "Price Watch" } });
    i18n.mergeLocaleMessage("zh", { price_watch: { label: "价格监控" } });
  }
});
